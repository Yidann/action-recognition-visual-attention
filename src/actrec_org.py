...
def param_init_lstm_cond(options, params, prefix='lstm_cond', nin=None, dim=None, dimctx=None):
    ...
    if options['selector']:
        ...
        W_csel = norm_weight(dimctx, 1)#dim_ctx 0518
        params[_p(prefix, 'W_csel')] = W_csel#new params 0518
...
def train(dim_out=100, # hidden layer dim for outputs
    ...
    if alpha_c > 0.:
        ...
        if selector:
            alpha_reg = alpha_c * ((1.-opts_out['selector'].sum(0))**2).mean()#0519 regularization     
        else: 
